 ...
 <form  method="post">
    ...
    <?php
         require_once 'config.php';
            ...
            $query = "SELECT kode_rak FROM rak ORDER BY kode_rak DESC LIMIT 1";
            $result = mysqli_query($conn, $query);
            $row = mysqli_fetch_assoc($result);
            $next_code = $row['kode_rak'] + 1; // Kode masuk berikutnya

            // Menampilkan input kode masuk berikutnya
            echo '<input type="text" id="kode_rak" name="kode_rak" value="' . $next_code . '" readonly><br>';
        ?>
    <label for="nama_rak">Nama Rak</label>
    <input type="text" id="nama_rak" name="nama_rak" required>
    
    <label for="kapasitas_rak">Kapasitas:</label>
    <input type="text" id="kapasitas_rak" name="kapasitas_rak" required>
    <br><tr>
    <div class="mb-3">
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $kapasitas_rak = $_POST['kapasitas_rak'];

    // Query untuk menyimpan data ke dalam database
    $query = "INSERT INTO rak (kode_rak, nama_rak, kapasitas_rak) VALUES ('$kode_rak', '$nama_rak', '$kapasitas_rak')";

    // Jalankan query
    if (mysqli_query($conn, $query)) {
        // // Jika berhasil disimpan, redirect ke halaman form_simpan.php
        // header("Location: formbuyer.php");
        exit();
