...
<h2 class="w3-text-grey w3-padding-16">DATA ARTIKEL</h2>
 ...
 <form method="post">
    ...
    <?php
         require_once 'config.php';
            ...
            $query = "SELECT kode_artikel FROM artikel ORDER BY kode_artikel DESC LIMIT 1";
            $result = mysqli_query($conn, $query);
            $row = mysqli_fetch_assoc($result);
            $next_code = $row['kode_artikel'] + 1; // Kode masuk berikutnya

            // Menampilkan input kode masuk berikutnya
            echo '<input type="text" id="kode_artikel" name="kode_artikel" value="' . $next_code . '" readonly><br>';
        ?>
    ...
    <input type="text" id="kode_slot" name="nama_slot" required> -->
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  // Query untuk mendapatkan kode_color berdasarkan nama_color
  $query_color = "SELECT kode_color FROM color WHERE nama_color = '$nama_color'";
  $result_color = mysqli_query($conn, $query_color);

  if (mysqli_num_rows($result_color) > 0) {
      // Jika color ditemukan, ambil kode_color
      $row_color = mysqli_fetch_assoc($result_color);
      $kode_color = $row_color['kode_color'];

      // Query untuk menyimpan data ke dalam tabel artikel
      $query_insert = "INSERT INTO artikel (kode_artikel, nama_artikel, kode_color, unit, lot) 
                       VALUES ('$kode_artikel', '$nama_artikel', '$kode_color', '$unit', '$lot')";
      ...
      echo "Nama color tidak ditemukan dalam database.";
  }
