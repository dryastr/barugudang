...
<h2 class="w3-text-grey w3-padding-16">DATA SLOT</h2>
<div class="w3-container">
<h5 class="w3-opacity"><b>isikan data dibawah ini dengan benar</b></h5>
 <form method="post">
    <label for="kode_slot">Kode Slot:</label>
    <?php
         require_once 'config.php';
            // Query untuk mendapatkan kode masuk terakhir
            $query = "SELECT kode_slot FROM slot ORDER BY kode_slot DESC LIMIT 1";
            $result = mysqli_query($conn, $query);
            $row = mysqli_fetch_assoc($result);
            $next_code = $row['kode_slot'] + 1; // Kode masuk berikutnya

            // Menampilkan input kode masuk berikutnya
            echo '<input type="text" id="kode_slot" name="kode_slot" value="' . $next_code . '" readonly><br>';
        ?>

    <label for="nama_slot">Nama Slot</label>
    <input type="text" id="nama_slot" name="nama_slot" required>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        $query_insert = "INSERT INTO slot (kode_slot, nama_slot, kode_rak) VALUES ('$kode_slot', '$nama_slot', '$kode_rak')";
