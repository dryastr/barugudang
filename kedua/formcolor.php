...
<h2 class="w3-text-grey w3-padding-16">DATA COLOR</h2>
<div class="w3-container">
<h5 class="w3-opacity"><b>isikan data dibawah ini dengan benar</b></h5>
 <form method="post">
    <label for="kode_color">Kode Color:</label>
    <?php
         require_once 'config.php';
            // Query untuk mendapatkan kode masuk terakhir
            $query = "SELECT kode_color FROM color ORDER BY kode_color DESC LIMIT 1";
            $result = mysqli_query($conn, $query);
            $row = mysqli_fetch_assoc($result);
            $next_code = $row['kode_color'] + 1; // Kode masuk berikutnya

            // Menampilkan input kode masuk berikutnya
            echo '<input type="text" id="kode_color" name="kode_color" value="' . $next_code . '" readonly><br>';
        ?>
    <label for="nama_color">Nama Color</label>
